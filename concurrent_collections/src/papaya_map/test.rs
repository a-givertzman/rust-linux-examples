...
pub struct PapayaMapTest {
    name: String,
}
impl PapayaMapTest {
    pub fn new(name: impl Into<String>) -> Self {
        Self {
            name: format!("PapayaMap {}", name.into()),
        }
    }
}
impl Test for PapayaMapTest {
    fn run(&self, receivers: usize, producers: usize, loads: usize, load_interval: Duration, data: Vec<Event>) -> Result<TestResult, Error> {
        ...
        log::debug!("PapayaMapTest.run | {} loads executed ", loads.len());
        let p_h: Vec<JoinHandle<()>> = producers.iter_mut().map(|p| p.run()).collect();
        log::debug!("PapayaMapTest.run | {} producers executed ", producers.len());
        for h in p_h {
            h.join().unwrap();
        }
        log::debug!("PapayaMapTest.run | {} producers exited ", producers.len());
        ...
        log::debug!("PapayaMapTest.run | {} loads exited ", loads.len());
        Ok(TestResult {
