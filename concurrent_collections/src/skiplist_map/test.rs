...
pub struct SkipListMapTest {
    name: String,
}
impl SkipListMapTest {
    pub fn new(name: impl Into<String>) -> Self {
        Self {
            name: format!("SkipListMap {}", name.into()),
        }
    }
}
impl Test for SkipListMapTest {
    fn run(&self, receivers: usize, producers: usize, loads: usize, load_interval: Duration, data: Vec<Event>) -> Result<TestResult, Error> {
        ...
        log::debug!("SkipListMapTest.run | {} loads executed ", loads.len());
        let p_h: Vec<JoinHandle<()>> = producers.iter_mut().map(|p| p.run()).collect();
        log::debug!("SkipListMapTest.run | {} producers executed ", producers.len());
        for h in p_h {
            h.join().unwrap();
        }
        log::debug!("SkipListMapTest.run | {} producers exited ", producers.len());
        ...
        log::debug!("SkipListMapTest.run | {} loads exited ", loads.len());
        Ok(TestResult {
